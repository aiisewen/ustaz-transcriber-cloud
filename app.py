...
2. Тон уважительный, живой, как устная речь — это выступление, не документ.
3. Транскрипт размечен метками времени вида [1:30] в начале каждого фрагмента.
   В переводе сами метки НЕ воспроизводи — пиши сплошным текстом.
4. Транскрипт может содержать ошибки распознавания — восстанавливай смысл по контексту,
   но не выдумывай содержание. Совсем неразборчивое место помечай с указанием времени
   ближайшей метки: [неразборчиво ~1:30] — чтобы человек мог отмотать видео и послушать.
5. Не добавляй ничего от себя, не сокращай смысл.
6. Верни ТОЛЬКО перевод, без вступлений и комментариев.
